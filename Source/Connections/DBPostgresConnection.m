...
static NSDataDetector *pgTimestampDetector = nil;
...
static dispatch_block_t createDateFormatter = ^{
    ...
    pgTimestampDetector = [[NSDataDetector alloc] initWithTypes:(NSTextCheckingTypes)NSTextCheckingTypeDate error:nil];
};
static NSString *NSDateToPostgresTimestamp(NSDate *date)
{
...
static NSDate *NSDateFromPostgresTimestamp(NSString *timestamp)
{
    ...
    NSDate *date = [pgDateFormatter dateFromString:timestamp];
    if(date)
        return date;

    return [[[pgTimestampDetector matchesInString:timestamp options:0 range:NSMakeRange(0, [timestamp length])] lastObject] date];
}